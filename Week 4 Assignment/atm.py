...
def atm(account_id, pin, action, amount=0):
    ...
    if pin != account["pin"]:
        ...
        return
    if action == "balance":
        print(f"{account['name']} balance is NPR {account['balance']}")
    elif action == "deposit":
        account["balance"] = account["balance"] + amount
        print(f"Deposit successful. New balance is NPR {account['balance']}")
    elif action == "withdraw":
        if amount > account["balance"]:
            print("Insufficient funds")
        else:
            account["balance"] = account["balance"] - amount
            print(f"Withdrawal successful. New balance is NPR {account['balance']}")
